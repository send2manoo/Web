...
(function($) {
  $.fn.drilldownSelect = function(params) {
    ...
    var defaults = {
      ...
      valueName: 'categoryName', // name of value parameter in data array
	...
	holder.css('width', '93%'); // Added to fix width issue
        isVisible = true;
      });
    });
	...
	if(path.length>1){
		...
		dataToSearch=fullTerriCategories;
	}
        var pathCurrentId = parseInt(path1);
        if (dataToSearch) {
          for (var i = 0; i < dataToSearch.length; i++) {
            if (dataToSearch[i].id == pathCurrentId) {
              pathValues += '<span value='+dataToSearch[i].categoryPositionCsv+'>'+dataToSearch[i].categoryName+'</span>';
              break;
            }
          }
        }
    ...
    function makeDropdown(element, event, path, show) {
      ...
      if(event==null || path==""){
		...
		for (var i = 0; i < pathArray.length; i++) {
	 	...
	 	data = fullTerriCategories;
      		if (pathArray.length) {
			if(pathArray.length>0)var pathArray1=pathArray[pathArray.length-1];
			//for (var i = 0; i < pathArray1.length; i++) {
		  		var pathArrayElement = parseInt(pathArray1);
		  		for (var j = 0; j < data.length; j++) {
		    			if (data[j].parentCat == pathArrayElement) {
		      			//data = data[j][defaults.listName];
		      			//break;
						if(data[j].hasChild==0){
							resData.push({"id": data[j].id,"categoryName": data[j].categoryName});
						}else{
							resData.push({"id": data[j].id,"categoryName": data[j].categoryName, "list": [{"id": data[j].id, "categoryName": "unknown"}]});
						}
		    			}
		  		}
			//}
